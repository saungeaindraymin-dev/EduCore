...
/** Total teaching time in a week, e.g. "6h 30m" */
...
export default function TeacherSchedulePage() {
  ...
  // Own courses, for the filter and to know which classes link to a course page
  const courseFilter = useApiQuery(`teacher-schedule:courses:${teacherId}`, () =>
    coursesApi.list({ teacherId, pageSize: 100 }),
  );
  const myCourseIds = new Set((courseFilter.data?.data ?? []).map((c) => c.id));

  // The API already limits a teacher to classes they teach or that belong to their courses
  const params = {
  ...
  const { data, error, isLoading, reload } = useApiQuery(
    `teacher-schedule:${JSON.stringify(params)}`,
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-sub">
          Your teaching week. Classes are scheduled by an administrator.
        </p>
      </header>
                  ...
                  return (
                    <li key={e.id}>
                      ...
                      >
                        ...
                        {e.teacher && e.teacher.id !== teacherId && (
                          <p className="truncate text-xs text-sub italic">
                            taught by {e.teacher.name}
                          </p>
                        )}
                      </button>
                    </li>
      ...
      <ClassDetailsDialog
        ...
        courseHref={
          selected && myCourseIds.has(selected.course.id)
            ? `/teacher/courses/${selected.course.id}`
            : null
        }
      />
    </div>
